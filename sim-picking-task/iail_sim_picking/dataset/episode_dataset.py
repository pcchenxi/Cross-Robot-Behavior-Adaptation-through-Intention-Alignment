...
# Reverse-sort task names so more specific names match first.
...
class RavensDataset(Dataset):
    ...
    def __init__(self, path, cfg, n_demos=0, augment=False):
        ...
        self.aug_theta_sigma = self.cfg['dataset']['augment']['theta_sigma'] if 'augment' in self.cfg['dataset'] else 60  # Fall back for older configs without theta_sigma.
        self.pix_size = 0.003125
        self.in_shape = (320, 160, 6)
        self.cam_config = cameras.RealSenseD415.CONFIG
        self.bounds = np.array([[0.25, 0.75], [-0.5, 0.5], [0, 0.28]])

        # Scan the existing dataset on disk, if present.
        ...
        if self.n_demos > 0:
            ...
            # Ensure the requested number of demos is available.
    ...
    def add(self, seed, episode):
        ...
        """
        joint_state, ee_state, color, depth, action, reward, info = [], [], [], [], [], [], []
        for obs, act, r, i in episode:
            joint_state.append(obs['joint_state'])
            ee_state.append(obs['ee_state'])
        ...
        dump(ee_state, 'ee_state')
        dump(joint_state, 'joint_state')
        dump(color, 'color')
        dump(depth, 'depth')
        dump(action, 'action')
        dump(reward, 'reward')
    ...
    def load(self, episode_id, images=True, cache=False):
        ...
        for fname in sorted(os.listdir(path)):
            if f'{episode_id:06d}' in fname:
                ...
                joint_state = load_field(episode_id, 'joint_state', fname)
                ee_state = load_field(episode_id, 'ee_state', fname)
                ...
                for i in range(len(action)):
                    obs = {'color': color[i], 'depth': depth[i], 'ee_state': ee_state[i], 'joint_state': joint_state[i]} if images else {}
    ...
    def process_sample(self, datum, augment=True):
        ...
        if act:
            ...
            if 'pose1' in act and act['pose1'] is not None:
    ...
    def __getitem__(self, idx):
        ...
        # Sequential tasks use the following step as the goal state.
...
class RavensMultiTaskDataset(RavensDataset):
    ...
    MULTI_TASKS = {
        # Task groups used for multi-task training.
        'multi-all': {
            'train': [
                'align-box-corner',
                'assembling-kits',
                ...
                'picking-unseen-google-objects-seq',
            ...
            'val': [
                ...
                'packing-unseen-google-objects-seq',
                'picking-seen-google-objects-seq',
                'picking-unseen-google-objects-seq',                
        ...
        # Language-conditioned task group.
        'multi-language-conditioned': {
            'train': [
                'align-rope',
                'assembling-kits-seq-unseen-colors',  # Uses the seen split in training to match the single-task setup.
                'packing-boxes-pairs-unseen-colors',
                'packing-shapes',
                'packing-unseen-google-objects-seq',
                'picking-unseen-google-objects-seq',
    ...
    def __init__(self, path, cfg, group='multi-all',
        ...
        if self.n_demos > 0:
            self.images = self.cfg['dataset']['images']
            self.cache = False  # Multi-task caching is disabled until task-aware caching is implemented.
            self.set(episodes)
    ...
    def load(self, episode_id, images=True, cache=False):
        if self.attr_train_task is None or self.mode in ['val', 'test']:
            ...
            all_tasks = [self.attr_train_task] + all_other_tasks

            # Sample the seen reference task with 50% probability, and
            # distribute the remaining probability across other tasks.
