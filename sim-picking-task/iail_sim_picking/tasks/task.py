...
from iail_sim_picking.tasks.grippers import Suction
...
class Task():
    ...
    def __init__(self):
        ...
        # Pose-matching thresholds used by evaluation.
        self.pos_eps = 0.01
    ...
    def oracle(self, env):
        ...
        def act(obs, info):  # pylint: disable=unused-argument
            ...
            # Use orthographic RGB-D images and segmentation masks for oracle actions.
            ...
            # Prioritize objects that are farthest from their nearest valid target.
            nn_dists = []
            nn_targets = []
            for i in range(len(objs)):
                object_id, (symmetry, _) = objs[i]
            ...
            pick_pose = (np.asarray(obj_pose[0]), np.asarray((0, 0, 0, 1)))
    ...
    def reward(self):
        ...
        if metric == 'pose':
            ...
            object_id, (symmetry, _) = objs[0]
            obj_pose = p.getBasePositionAndOrientation(object_id)[0]
            if obj_pose[2] > 0.2:
    ...
    def is_match(self, pose0, pose1, symmetry):
        ...
        # Compute z-axis rotational error while accounting for symmetry.
        diff_rot = 0
        if symmetry > 0:
