...
def eulerXYZ_to_quatXYZW(rotation):  # pylint: disable=invalid-name
    ...
    """
    return p.getQuaternionFromEuler(rotation)
...
def quatXYZW_to_eulerXYZ(quaternion_xyzw):  # pylint: disable=invalid-name
    ...
    Returns:
      rotation: a 3-parameter rotation, in xyz order, tuple of 3 floats
    """
    return p.getEulerFromQuaternion(quaternion_xyzw)
...
def preprocess(img, dist='transporter'):
    ...
    # Select normalization statistics for the requested input distribution.
    if dist == 'clip':
    ...
    # Convert statistics to tensors when normalizing torch inputs.
...
def get_se3_from_image_transform(theta, trans, pivot, heightmap, bounds,
    ...
    t_centernew_center[0:3, 0:3] = np.array(
        [
...
def _require_meshcat():
    if meshcat is None:
        raise ModuleNotFoundError(
            "meshcat is required for visualization utilities but is not installed."
        )
...
def load_hydra_config(config_path):
    if OmegaConf is None:
        raise ModuleNotFoundError(
            "omegaconf is required for Hydra config loading but is not installed."
        )
